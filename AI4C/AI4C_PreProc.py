...
# Set number of days considered training the model,
# in order to prevent target leakage during model training.
num_day = 30
# Update training data consider only the demands in the last num_day
max_day = tr_df['day'].max()
tr_clean_df = tr_df.copy()
if max_day - num_day >= 0:
    tr_clean_df = tr_df.loc[tr_df['day'] > max_day - num_day]
    tr_clean_df['day'] -= max_day - num_day

# Update training data consider only the demands in the first num_day
tr_df = tr_df.loc[tr_df['day'] < num_day]
...
tr_clean_df.insert(tr_clean_df.shape[1], 'time', 0)
tr_clean_df['time'] = ((tr_clean_df['day'] - 1) * (4 * 24) +
                       tr_clean_df['timestamp']).astype(int)
...
tr_clean_df.insert(tr_clean_df.shape[1],
                   't_demand', value=tr_clean_df['demand'])
tr_clean_df.insert(tr_clean_df.shape[1],
                   't_time', value=tr_clean_df['time'].astype(int))
tr_clean_df.insert(tr_clean_df.shape[1],
                   't_geo', value=tr_clean_df['geohash6'])
...
# information.
tr_clean_df.sort_values(by=['geohash6', 'time'],
                        ascending=True,
                        inplace=True)
tr_clean_df.reset_index(drop=True, inplace=True)

# Ensure that the future demand is belong to the particular location and time,
# then proceed setting the future demand.
tr_clean_df[['t_demand', 't_time', 't_geo']] = \
    tr_clean_df[['t_demand', 't_time', 't_geo']].shift(-1)
...
tr_clean_df.loc[(tr_clean_df['time'] + 1 != tr_clean_df['t_time']) |
                (tr_clean_df['geohash6'] != tr_clean_df['t_geo']),
                't_demand'] = 0.0

# Remove unnecessary columns used during the incorporation of future demand
# data.
tr_clean_df.drop(['t_time', 't_geo'], axis=1, inplace=True)

# Clean data from any NaN value.
tr_clean_df.dropna(inplace=True)

# Save the pre-processed training data into csv file.
tr_clean_df.to_csv(cwd + '/Traffic data/prefinal_training.csv', sep=',')
